...
class AboutGenerators(Koan):
    ...
    def test_generator_expressions_are_a_one_shot_deal(self):
        ...
        #second attempt give empty list
    ...
    def test_generators_can_be_manually_iterated_and_closed(self):
        ...
        self.assertEqual('peanut', next(result))#0
        #actually here last element in the result have to printed but in generaters it follows the sequesnce
        self.assertEqual('butter', next(result))#1
        print(next(result))#2
        result.close()
